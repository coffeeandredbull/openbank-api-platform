...
> Slice 4) that is fully isolated from the runtime request path, and a
> JWT-protected query endpoint over those persisted events (Phase 23, Slice 5),
> plus a JWT-protected usage summary endpoint computed in PostgreSQL
> (Phase 23, Slice 6))
...
- **Gateway → API Management (Phases 17 and 21):** the gateway calls the API
  ...
  returns credential + subscription verdict in one call; since Phase 24 Slice 3
  it authenticates **only `ACTIVE`** credentials). These are the only
...
- **Tier lifecycle / tier administration**: the ADMIN-only partial update
  `PATCH /subscription-tiers/{tierId}` and safe deletion of unreferenced tiers
  via `DELETE /subscription-tiers/{tierId}` are implemented (Phase 24 Slices 8
  and 9); creation/listing, lifecycle, and pricing/hard limits remain planned.
...
60s default TTL via `spring.cache.redis.time-to-live`). Read path is
   ...
   in-place mutations of cached values are the API version lifecycle change and
   the subscription tier update; each evicts exactly its own
   `apiVersion::<apiId>::<versionId>` or `subscriptionTier::<tierId>` key **after
   commit** via `CacheInvalidationService` (Slices 7/8 hardening; whole-cache
