...
| **API Management Service** | Owns the API catalog: published APIs, versions, endpoint metadata, documentation, and lifecycle state (published / deprecated / retired). It is the source of truth for "which API versions exist". Also owns the **developer application registry** (Phase 11) and the **subscription registry** (Phase 12): what an application subscribes to. Ownership of both is enforced from JWT claims — the service never trusts a client-supplied owner. |
| **Subscription Service** | Owns subscription **credentials** and **tiers**. Links a subscription (created in the API Management Service) to provisioning of credentials (API key / client ID + secret) and a rate-limit tier, and enforces whether an application is actually allowed to call an API. The Application → Subscription → API Version linking itself is implemented in the API Management Service (Phase 12). |
